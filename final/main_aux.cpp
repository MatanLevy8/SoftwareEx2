...
void importSorted(int index, double distance, keyValue** items)
{
	...
	if (j < NUM_OF_BEST_DIST_IMGS)
	{
		...
		// clear memory of the previous last item
		if (limit == NUM_OF_BEST_DIST_IMGS)
			free(temp1);
	}
	else
...
void searchSimilarImages(char* imagePath)
{
	...
	imageData* workingImageData = calcImageData(imagePath, allSettings->numOfBins,
			allSettings->numOfFeatures);

	// verify database is built (logically we could build the database after setting the
	//							images paths but if the user will choose to exit at first
	// 							run would then work for nothing)
	if (allImagesDatabase == NULL)
		setImagesDatabase();
...
void start()
{
	...
	// initiate allSettings according to user input
	setInput();
